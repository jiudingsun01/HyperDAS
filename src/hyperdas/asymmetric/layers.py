...
class HyperDASCrossAttention(LlamaAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.is_cross_attention:
            ...
            kv_position_ids.view(self.encoder_hidden_states_num_layers, -1)
...
class HyperDASDecoderLayerWithDoubleCrossAttention(LlamaDecoderLayer):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        if not self.ablate_source_token_attention:
            self.source_cross_attn = HyperDASCrossAttention(config=config, layer_idx=layer_idx, is_cross_attention=True)
        ...
        if not self.ablate_base_token_attention:
            self.base_cross_attn = HyperDASCrossAttention(config=config, layer_idx=layer_idx, is_cross_attention=True)
...
class HyperDASDecoderLayerWithCrossAttention(LlamaDecoderLayer):
    def __init__(self, config: LlamaConfig, layer_idx: int, add_cross_attention=False):
        super().__init__(config, layer_idx)
        if add_cross_attention:
            self.cross_attn = HyperDASCrossAttention(config=config, layer_idx=layer_idx, is_cross_attention=True)
