...
def split_axbench16k_train_test(axbench_train_set, split_by="concept", train_ratio=0.8):
    ...
    if split_by == "concept":
        ...
        train_concept_ids = set(random.sample(all_concept_ids, int(len(all_concept_ids) * train_ratio)))
        train_set = [d for d in axbench_train_set if d["concept_id"] in train_concept_ids]
        test_set = [d for d in axbench_train_set if d["concept_id"] not in train_concept_ids]
    
    elif split_by == "example":
        raise NotImplementedError
    
    elif split_by == "random":
        train_set = random.sample(axbench_train_set, int(len(axbench_train_set) * train_ratio))
...
def tokenize_text_inputs(tokenizer, inputs, targets, add_space_before_target=True):
    ...
    if add_space_before_target:
        ...
        for ipt, targ in zip(inputs, targets):
            if ipt.endswith(" ") or ipt.endswith("\"") or ipt.endswith("\'") or ipt.endswith("("):
